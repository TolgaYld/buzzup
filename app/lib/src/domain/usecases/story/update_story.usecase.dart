import 'package:buzzup/core/common/domain/entities/all_entities.dart';
import 'package:buzzup/core/enums/content_visibility.dart';
import 'package:buzzup/core/usecases/usecases.dart';
import 'package:buzzup/core/utils/typedefs.dart';
import 'package:buzzup/src/domain/repositories/post/post.repo.dart';

class UpdatePostUsecase extends UsecaseWithParams<void, PostEntity> {
  const UpdatePostUsecase(this._repo);
  final PostRepo _repo;

  @override
  ResultFuture<void> call(PostEntity params) async => await _repo.update(params);

  static PostEntity get emptyParams => PostEntity(
        id: "empty_id",
        channels: [ChannelEntity.empty()],
        visibility: ContentVisibility.public,
        location: LocationEntity.empty(),
        createdAt: DateTime.parse('2024-02-10T14:38:36.936Z'),
        createdBy: UserEntity.empty(),
        isActive: true,
        isDeleted: false,
      );
}
